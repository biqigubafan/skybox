Shader "Toon/URP_Toon"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {   
                //lightmap，4个通道分别是，r:高光范围，g:AO mask b:高光强度 a:ramp的y坐标
                ...
                //接下来计算ramp的坐标，x坐标按照hanllambert,y坐标按照a通道存储的阈值
                float RampPixelX = 0.00195;  // = 1/256/2
                float RampPixelY = 0.02500;  //0.03125 = 1/20/2   尽量采样到ramp条带的正中间，以避免精度误差
                float RampX, RampY;
                // 对X做一步Clamp，防止采样到边界
                ...
                // 最终计算，_RampShadowRange是阈值，自己设置，没到达就用rampColor * BaseColor * _ShadowColor，不然就BaseColor
               ...
               //— — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — — —
               // SDF脸部阴影，根据方向光和右手方向，计算sdf的采样点，然后拿到阈值，再根据方向光和面朝方向，计算实际颜色
                #if _SHADERENUM_FACE
                ...
                half4 StepSpecular;
                half4 FinalSpecular;
                // ILM的R通道，灰色为裁边视角高光，0-0.8部分只加视角光
                ...
                // 金属高光，把世界空间发现转到view空间，然后使用xy坐标在metalmap上采样，然后根据阈值加上金属高光
                ...
                // 法线外扩偏移UV，把smothNormal转换到世界空间,再转换到视角空间，记得除i.pos.w，偏移后的值用来乘smoothstep
        ...
        Pass
        {
            ...
            //跟边缘光差不多，就是在另一个pass里，然后要等距离，发生在顶点着色器，用了正面剔除
