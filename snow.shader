Shader "Toon/Lit Snow" {
    ...
    SubShader{
        ...
        void surf(Input IN, inout SurfaceOutput o) {
            // Effects RenderTexture Reading 又他妈计算一遍
            ...
            //顶点不能LOD，surf可以，这回用的zx提取，不一样了
            float3 noisetexture = tex2D(_Noise, IN.worldPos.zx * _NoiseScale);
 
            //顶点不能LOD，surf可以
            float snowtexture = tex2D(_MainTex, IN.worldPos.zx * _SnowTextureScale).r;
            ...
            float baseNoise = tex2D(_Noise, (IN.worldPos.zx - _Time.x*10) *_NoiseScale).x;
            // cutoff and where there is no path
            float cutoffSparkles = step(_SparkCutoff,sparklesStatic  *(1- saturate(effect.g))*baseNoise);         
